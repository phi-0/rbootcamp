...
fig3 <- fig3 %>% layout(autosize = TRUE)

#fig3 <- fig3 %>% layout(autosize = FALSE, width = 800, height = 500, margin = list(l=50, r=10, b=10, t=10, pad=1))
#htmlwidgets::saveWidget(fig3, 'fig3.html')

fig3
```

Knitr for some reason will not render this interactive, animated plot even though it renders perfectly well when executing the single chunk in PyCharm. It appears there is some issue with rendering HTML widgets from plotly. After hours of debugging, no clear reason or solution can be found. As a work-around, the resulting figure has been exported as a HTML document including relevant widgets. The plot can be access via this link:

<center>
[**>>>LINK TO ANIMATED PLOT<<<**](../fig3.html)
</center>
