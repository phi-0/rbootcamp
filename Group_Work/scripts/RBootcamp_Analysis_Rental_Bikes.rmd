...
```{r, message = FALSE, warning = FALSE, cache = TRUE, animation.hook = 'gifski'}
...
Knitr for some reason will not render this interactive, animated plot. It appears there is some issue with rendering HTML widgets from plotly. After hours of debugging, no clear reason or solution can be found. As a work-around, the resulting figure has been exported as a HTML document including relevant widgets. The plot can be access via this link:
