...
In the context of the R-Bootcamp we have decided to analyze and work with a data set provided by the city of Seoul containing hourly bike rental data. Rental bikes have become a popular means of transportation. They are easily accessible, cheap and  practical. For providers it is important to predict the demand for their bikes in order to plan ahead and control their costs effectively. The data set does not only contain information about the number of bikes rented hourly but also about the weather. For the purpose of practicing the merger of multiple data sets we have decided to 
extend our data with information from an additional source. The second source consists of the hourly number of arrivals at the Incheon Airport of Seoul. This addition will allow us to expand our analysis and test whether there is evidence that the arrivals have an effect on the number of bikes rented. Our analysis will be structured as follows: First we will load the data, then we will check if any transformations are necessary and we will merge the two data sets. In the next step we will try to get first insights to the data by creating graphs. On the basis of the graphs we will then implement a simple and a complex model that can be used for predictions. After comparing the two models we will use plotly, a package not discussed in the course, in order to create a 3-dimensional graph.
